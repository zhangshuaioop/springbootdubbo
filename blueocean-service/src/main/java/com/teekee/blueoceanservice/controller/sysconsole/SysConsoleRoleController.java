...
import com.teekee.blueoceanservice.service.sysconsole.SysConsoleRoleService;
...
public class SysConsoleRoleController {
    ...
    public Result updateSave(@RequestBody SysConsoleRole sysConsoleRole) {
        ...
        if(sysConsoleRole != null && sysConsoleRole.getId() != null){
            logger.info("进入更新操作");
            result = ResultUtil.success(sysConsoleRoleService.update(sysConsoleRole));
        }else {
            logger.info("进入保存操作");
            sysConsoleRole.setCreatePerson(sysConsoleUsers.getId());
            result = ResultUtil.success(sysConsoleRoleService.save(sysConsoleRole));
        }
