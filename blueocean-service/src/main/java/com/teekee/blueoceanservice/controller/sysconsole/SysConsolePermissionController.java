...
public class SysConsolePermissionController {
    ...
    public Result findTree(@RequestBody SysConsolePermission sysConsolePermission) {
        logger.info("平台权限列表(树)开始,入参:sysConsolePermission="+sysConsolePermission.toString());
        Result result = sysConsolePermissionService.findByRoleIdList(sysConsolePermission);
        logger.info("平台权限列表(树)结束,出参:result="+result.toString());
        return result;
    }
