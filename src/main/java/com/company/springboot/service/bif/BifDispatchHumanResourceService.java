...
import com.company.springboot.mapper.bif.BifDispatchHrAccountInfoMapper;
import com.company.springboot.mapper.bif.BifDispatchHrAutonymValidMapper;
...
import com.company.springboot.utils.ValiDateUtil;
...
public class BifDispatchHumanResourceService {
    ...
    public Result list(DispatchHumanResourceList dispatchHumanResourceList) {
        ...
        dispatchHumanResourceList.setUserId(userId);
        // 查询操作者派工过的人员集合
        List<BifDispatchHumanResource> sqlResult = null;
        if (dispatchHumanResourceList.getFlagMyUsed()) {
            dispatchHumanResourceList.setDealPersonId(userId);
        ...
        if (dispatchHumanResourceList.getFlagMyUsed() && (sqlResult == null || sqlResult.size() == 0)) {
            List<BifDispatchHumanResource> list = new ArrayList<>();
            PageInfo<BifDispatchHumanResource> persons = new PageInfo<>(list);
            if(persons.getList()==null||persons.getList().size()==0){
    ...
    public Result serviceHandle(BifDispatchHumanResource serviceHumanResource) {
        ...
        if(!ValiDateUtil.failPhone(serviceHumanResource.getMobile())){
            return ResultUtil.validateError("请输入正确手机号！");
        }

        if (serviceHumanResource.getId() == null || serviceHumanResource.getId() == 0) {
     ...
     * @Date：2018/10/24
     */
    public Result checkHrMobile(String mobile,String type) {
        List<BifDispatchHumanResource> humanResources = mapper.selectByMobile(mobile);
        if(!ValiDateUtil.failPhone(mobile)){
            return ResultUtil.validateError("请输入正确手机号！");
        }
        if(type.equals("add")){
            if (humanResources.size() > 0) {
