package com.company.springboot.controller.sys;

import com.company.springboot.entity.sys.SysCompanyPermission;
import com.company.springboot.service.sys.SysCompanyPermissionService;
...
public class SysCompanyPermissionController {
    ...
    public Result findPage(@RequestBody SysCompanyPermission sysCompanyPermission) {
        ...
        logger.info("分页公司权限列表开始,入参:sysCompanyPermission="+sysCompanyPermission.toString());
        Result result = sysCompanyPermissionService.findPage(sysCompanyPermission);
        logger.info("分页公司权限列表结束,出参:"+result.toString());
        return result;
    }
    ...
    public Result findList(@RequestBody SysCompanyPermission sysCompanyPermission) {

        logger.info("公司权限列表(树)开始,入参:sysCompanyPermission="+sysCompanyPermission.toString());
        Result result = sysCompanyPermissionService.findByRoleIdList(sysCompanyPermission);
    ...
    public Result saveUpdate(@RequestBody SysCompanyPermission sysCompanyPermission) {
        ...
        if(sysCompanyPermission != null){
            ...
            if(sysCompanyPermission.getId() != null && sysCompanyPermission.getId() > 0){
                ...
                logger.info("进入保存业务");
                sysCompanyPermission.setFlagDeleted(true);
