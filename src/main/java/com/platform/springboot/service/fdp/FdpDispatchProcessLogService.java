...
import com.platform.springboot.mapper.fdp.FdpDispatchProcessLogMapper;
import com.platform.springboot.mapper.fdp.FdpFaultDispatchOrderMapper;
import com.platform.springboot.mapper.fdp.FdpFaultDispatchTransferLogMapper;
        ...
        if (process.size() > 0 && order != null) {
            ...
            for (int i = 0; i <= process.size(); i++) {
                if (i < process.size()) {
                    ...
                    userName = sysCompanyUsersMapper.selectByPrimaryKey(process.get(i).getDealPersonId()).getUsername();
                    ...
                    if (order.getDispatchStatus().equals("NEEDCONFIRM") || order.getFlagDispatcherConfirm() == true) {
                        userName = sysCompanyUsersMapper.selectByPrimaryKey(request.getUpdatePerson()).getUsername();
                    } else {
                        userName = sysCompanyUsersMapper.selectByPrimaryKey(order.getLastDealPersonId()).getUsername();
                    }
                    show = new FdpDispatchProcessLogShow();
    ...
    public List<FdpFaultDispatchTransferLogShowResp> FdpFaultDispatchTransferLogShowList(int dispatchOrderId) {
        ...
        for (FdpFaultDispatchTransferLog item : sqlResult) {
            ...
            if (item.getOrgDealPersonId() == null || item.getOrgDealPersonId() == 0) {
                String name = sysCompanyUsersMapper.selectByPrimaryKey(item.getNewDealPersonId()).getUsername();
                outItem.setInfo("【" + name + "】  接收派工本");
            } else {
                String oldName = sysCompanyUsersMapper.selectByPrimaryKey(item.getOrgDealPersonId()).getUsername();
                String newName = sysCompanyUsersMapper.selectByPrimaryKey(item.getNewDealPersonId()).getUsername();
