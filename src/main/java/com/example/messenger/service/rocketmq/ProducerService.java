    ...
    public Result producer(Map map){
        ...
        for (int i = 0; i < 1; i++) {
            ...
            try {
                ...
                logger.info(new Date() + " 发送mq消息失败. Topic为:" + msg.getTopic());
                logger.info("消息处理失败，进入redo日志表，待处理状态");
                ExceptionRedoLogs exceptionRedoLogs = new ExceptionRedoLogs();
                exceptionRedoLogs.setMessageId(map.get("messageId").toString());
                exceptionRedoLogs.setRequestBody(jsonObjectStr);
                exceptionRedoLogs.setRequestUrl("producerService/producer");
