package com.example.identification.filter;

import com.example.identification.entity.sys.SysUsersAccount;
import com.example.identification.utils.JwtTokenUtils;
...
//            }
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(String tokenHeader,HttpServletResponse response) throws IOException {
        ...
        SysUsersAccount user = JwtTokenUtils.getUser(token);
        if(account.equals("")){
            response.setContentType("application/json;charset=UTF-8");
            PrintWriter out = response.getWriter();
            out.write("{\"code\":\"403\",\"msg\":\"授权失败!\"}");
        ...
        return new UsernamePasswordAuthenticationToken(account, user, collection);
