...
import com.teekee.messengerservice.entity.message.Sms;
import com.teekee.messengerservice.entity.trc.TrcEmailLog;
import com.teekee.messengerservice.enums.BusinessEnum;
import com.teekee.commoncomponets.utils.RedisLockUp;
import com.teekee.commoncomponets.utils.RedisUnLock;
import com.teekee.messengerservice.service.email.EmailService;
import com.teekee.messengerservice.service.sms.SmsService;
        ...
        consumer.subscribe(
                ...
                new MessageOrderListener() {
                    ...
                    public OrderAction consume(Message message, ConsumeOrderContext context) {
                        ...
                        try {
                            //加锁
                            Boolean lock = RedisLockUp.tryGetDistributedLock(jedis, lockKey,requestId , 10000);

                            if(lock) {
                                //加锁成功
                                logger.info("加锁成功开始消费! msgId=" + message.getMsgID() + ",Body=" + new String(message.getBody()));
                                ...
                                if (map.get("type").equals(BusinessEnum.EMAIL.getType())) {
                                    //邮箱
                                    TrcEmailLog trcEmailLog = (TrcEmailLog) JSONObject.toBean(jsonobject, TrcEmailLog.class);
                                    emailService.sendEmail(trcEmailLog);
                                } else if (map.get("type").equals(BusinessEnum.SMS.getType())) {
                                    //短信
                                    Sms sms = (Sms) JSONObject.toBean(jsonobject, Sms.class);
